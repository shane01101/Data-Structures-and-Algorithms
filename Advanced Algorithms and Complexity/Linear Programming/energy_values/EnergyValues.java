...
class EnergyValues {
    ...
    {
        double curPivotVal = a[pivot_element.raw][pivot_element.column];
        
        //Divide row by the pivot val which trying to solve for so it is 1
        for(int i = 0; i < a[pivot_element.raw].length; i++)
        {
            a[pivot_element.raw][i] = a[pivot_element.raw][i] / curPivotVal;
        }
        b[pivot_element.raw] = b[pivot_element.raw] / curPivotVal;
        
        //Add or subtract from all other rows to get other column terms to 0
        for(int i = 0; i < a.length; i++)
        {
            double mult = a[i][pivot_element.column];
            if(i != pivot_element.raw) //ignore pivot row
            {
                for(int j = 0; j < a[pivot_element.raw].length; j++)
                {
                    a[i][j] -= a[pivot_element.raw][j] * mult;
                }
                b[i] -= b[pivot_element.raw] * mult;
            }
        }
    }
